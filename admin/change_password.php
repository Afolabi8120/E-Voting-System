    ...
    if(isset($_SESSION['email'])){
        if(isset($_POST['update'])){
            ...
            elseif($new_password != $new_password2){
                ...
                $sql = "SELECT * FROM tblregister WHERE email = '{$_SESSION['email']}' AND usertype = 'Admin'";
                ...
                if($result > 0){
                    while($row = mysqli_fetch_array($query_result)){
                        ...
                        if(password_verify($old_password, $pass)){
                            // Hashimg the new password provided by user
                            $new_pass = password_hash($new_password, PASSWORD_DEFAULT);
                                // Updating the new password
                                $sql = "UPDATE tblregister SET password = '$new_pass' WHERE email = '{$_SESSION['email']}' AND usertype = 'Admin'";
                                $query_result = mysqli_query($conn, $sql);
                                ?>
